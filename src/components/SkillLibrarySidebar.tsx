  ...
  return (
    <div className="rounded-xl border border-border bg-surface/60 p-4">
      <div className="mb-3 flex items-center justify-between">
        ...
        <Link to="/skills" className="text-[11px] accent-text hover:underline">
          View all →
        </Link>
      </div>
      ...
      {skills.length === 0 ? (
        <p className="text-xs text-muted-foreground">
          Empty. Trigger an agent to commit the first skill.
        </p>
      ) : (
        <ul className="space-y-1 max-h-[280px] overflow-auto scrollbar-thin">
          {skills.slice(0, 12).map((s) => (
            <li key={s.id} className="mono text-[11.5px] flex justify-between gap-2">
              ...
              {s.scheduled_cron && (
                <span className="text-[10px] rounded bg-primary/20 px-1 accent-text">cron</span>
              )}
            </li>
          ))}
        </ul>
